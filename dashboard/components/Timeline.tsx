...
import type { Thought, ActionResult } from "../../shared/types.js";
  ...
  return (
    <div style={{ marginBottom: 8 }}>
      ...
      <div
        ...
        onMouseLeave={() => !dragging && setHoveredStep(null)}
        style={{
          height: 32,
          background: "#21262d",
          borderRadius: 4,
      ...
      >
        ...
        {actions.map((a) => {
          ...
          return (
            <div
              key={a.step}
              title={`Step ${a.step}: ${a.action.action} ${isError ? "❌" : "✓"}`}
              style={{
                position: "absolute",
                left: `${((a.step - 1) / Math.max(totalSteps - 1, 1)) * 100}%`,
                top: "50%",
      ...
      {hoveredStep && !dragging && (
        ...
        >
          ...
          {hoveredAction && (
            <span>
              Action: <strong style={{ color: "#c9d1d9" }}>{hoveredAction.action.action}</strong>
              {!hoveredAction.success && <span style={{ color: "#f85149" }}> (failed)</span>}
            </span>
          )}
          {hoveredThought && (
            <span>
              ...
              <strong style={{ color: "#c9d1d9" }}>
                {(hoveredThought.confidence * 100).toFixed(0)}%
              </strong>
            </span>
          )}
        </div>
