...
export default function ThoughtBubble({ thought, action }: Props) {
  ...
  const confColor =
    thought.confidence > 0.7 ? "#3fb950" : thought.confidence > 0.4 ? "#d2991d" : "#f85149";
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <span style={{ fontSize: 13, fontWeight: 600, color: "#f0f6fc" }}>
            Agent Thought — Step {thought.step}
          </span>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span
              ...
              {action.result && (
                <div
                  style={{
                    fontSize: 12,
                    marginTop: 4,
                    color: action.success ? "#3fb950" : "#f85149",
                  }}
                >
                  {action.success ? "✓" : "✗"} {action.result.slice(0, 150)}
                </div>
              )}
            </div>
          )}
